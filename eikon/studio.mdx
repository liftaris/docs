...
- If the draft has unsaved changes (or no saved `.eikon` exists yet), Studio shows a **Save before submit?** prompt. Choose to save the current preview, discard and submit the last saved artifact, or cancel and keep editing.
...
After the gate, Studio opens the same **Submit eikon** dialog described in [Gallery](/eikon/gallery#submit-a-local-draft-to-the-official-registry). Fill in the public metadata, preflight the registry bundle, consent, then submit through the GitHub backend or use the manual PR fallback.
