...
<Warning>
  **Hermes Agent must be installed before you launch Herm.** Herm is a TUI client for the Hermes Agent gateway — it does not bundle or replace the agent runtime. Visit the [Hermes Agent repository](https://github.com/NousResearch/hermes-agent) and complete its setup first, including confirming that your Hermes home directory exists at `~/.hermes` (or that `HERMES_HOME` is set to its location).
</Warning>
