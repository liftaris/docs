...
As a long session approaches the model's context limit, the context gauge in the sidebar will fill toward capacity. Run `/compress` to let Hermes Agent summarize older turns and free up context window space without starting a new session. You can also use **`<leader>c`** as a keyboard shortcut for this action.
