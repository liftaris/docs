...
<Steps>
  ...
  <Step title="Open task detail">
    Press `Enter` to inspect title, body, assignee, run history, diagnostics, parent and child links, comments, attachments, and worker-log tail. The attachments block shows filename, size, and age for each file. Attachments inside the board's configured attachment root render as clickable `FileLink` entries. Attachments outside that root still appear in the list, but Herm does not link their absolute paths. Herm only reads attachments — add or remove files through Hermes Agent.
  </Step>
