...
The Hermes CLI (`hermes kanban <verb>`) executes structured status transitions (complete, block, unblock, archive, assign, dispatch) and ensures all associated lifecycle steps fire correctly.
...
Profile names must match the pattern `[a-z0-9][a-z0-9_-]{0,63}`. Names like `hermes`, `default`, `test`, `tmp`, `root`, and `sudo` are reserved. All write operations (create, delete, rename, export, use) route through the Hermes CLI, which authoritatively handles validation, skill seeding, and wrapper alias management.
